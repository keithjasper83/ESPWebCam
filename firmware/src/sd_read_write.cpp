 ...
 * sd_read_write.cpp
 *
 * SD card utility functions – compiled only when ENABLE_SD_CARD is defined.
 *
 * NOTE: The /video directory is no longer deleted or recreated at boot.
 * Recording to SD is not part of the primary workflow; all user-initiated
 * recording is performed by the iOS application using AVFoundation.
 */
...
void testFileIO(fs::FS &fs, const char * path){
    ...
    if(file){
        ...
        while(len){ size_t toRead = len > 512 ? 512 : len; file.read(buf, toRead); len -= toRead; }
...
int readFileNum(fs::FS &fs, const char * dirname){
    ...
    int num = 0;
    File file = root.openNextFile();
    while(file){ file = root.openNextFile(); num++; }
    return num;
}
