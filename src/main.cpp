...
const char* ssid     = "KJ iPhone";   //input your wifi name
const char* password = "hello123";   //input your wifi passwords
...
void cameraInit(void){
  ...
  config.xclk_freq_hz = 20000000;  // Increase to 20 MHz for better frame capture
  config.frame_size = FRAMESIZE_VGA;  // 640x480 for better quality
  config.pixel_format = PIXFORMAT_JPEG; // for streaming
  config.grab_mode = CAMERA_GRAB_WHEN_EMPTY;
  config.fb_location = CAMERA_FB_IN_PSRAM;
  config.jpeg_quality = 12;  // Increase quality (1-63, higher=better but larger)
  config.fb_count = 1;
